...
library(data.table)
# ======= READ INPUT DATA ============
inp_movies_df<-read.csv("movies_1M.dat")
inp_ratings_df<-read.csv("ratings_1M.dat")
...
#ignore films and users own correlation
diag(cor_films) <- NA 
diag(cor_users) <- NA
...
# lowest_cor <- sort(cor_films[,1],decreasing = TRUE)[nbr_it_user_consider]
# 
# cor_films_filt <- cor_films[,1]
cor_films_filt <- replace(cor_films, cor_films<0.2, NA)

sum(cor_films_filt[,1]*movies_pivoted[1,-1], na.rm=T)/sum(cor_films_filt[!is.na(movies_pivoted[1,-1]),1],na.rm=T) # user 1 , film 1 
sum(cor_films_filt[,1]*movies_pivoted[2,-1], na.rm=T)/sum(cor_films_filt[!is.na(movies_pivoted[2,-1]),1],na.rm=T) # user 2 , film 1 
sum(cor_films_filt[,2]*movies_pivoted[1,-1], na.rm=T)/sum(cor_films_filt[!is.na(movies_pivoted[1,-1]),2],na.rm=T) # user 1 , film 2 
sum(cor_films_filt[,2]*movies_pivoted[2,-1], na.rm=T)/sum(cor_films_filt[!is.na(movies_pivoted[2,-1]),2],na.rm=T) # user 2 , film 2

# == all users prediction for one film
cor_matrix_film <- sweep(movies_pivoted[,-1], MARGIN=2, cor_films_filt[,1], `*`)
cor_matrix_weights <- sweep(films_watch[,-1], MARGIN=2, cor_films_filt[,1], `*`)

rowSums(cor_matrix_film, na.rm = T) / rowSums(cor_matrix_weights, na.rm = T)
...
for (film in  target_films_present){ #colnames(movies_pivoted %>% select(-userId))
  ...
  cor_matrix_film <- sweep(movies_pivoted[,-1], MARGIN=2, cor_films_filt[,film], `*`)
  cor_matrix_weights <- sweep(films_watch[,-1], MARGIN=2, cor_films_filt[,film], `*`)
...
cor_users_filt <- replace(cor_users, cor_users<0.2, NA)

# == one movie and one user prediction
sum(cor_users_filt[,1]*movies_pivoted_tran_filt[1,], na.rm=T)/sum(cor_users_filt[!is.na(movies_pivoted_tran_filt[1,]),1],na.rm=T) # user 1 , film 1 
sum(cor_users_filt[,1]*movies_pivoted_tran_filt[2,], na.rm=T)/sum(cor_users_filt[!is.na(movies_pivoted_tran_filt[2,]),1],na.rm=T) # user 1 , film 2 
sum(cor_users_filt[,2]*movies_pivoted_tran_filt[1,], na.rm=T)/sum(cor_users_filt[!is.na(movies_pivoted_tran_filt[1,]),2],na.rm=T) # user 2 , film 1 
sum(cor_users_filt[,2]*movies_pivoted_tran_filt[2,], na.rm=T)/sum(cor_users_filt[!is.na(movies_pivoted_tran_filt[2,]),2],na.rm=T) # user 2 , film 2
...
cor_matrix_film <- sweep(movies_pivoted_tran_filt, MARGIN=2, cor_users_filt[,1], `*`)
cor_matrix_weights <- sweep(films_watch_tran_filt, MARGIN=2, cor_users_filt[,1], `*`)
...
for (user in colnames(movies_pivoted_tran) ){
  i=i+1
  if (i%%100 ==0){print(i)}
  cor_matrix_film <- sweep(movies_pivoted_tran_filt, MARGIN=2, cor_users_filt[,user], `*`)
  cor_matrix_weights <- sweep(films_watch_tran_filt, MARGIN=2, cor_users_filt[,user], `*`)
...
#saveRDS(pred_note_mat_user, file="pred_note_mat_user")
...
total_rmse <- sqrt(sum(error_mat_user**2,  na.rm = T)/sum(!is.na(error_mat_user)))
total_R2 <- 1 - total_rmse**2 / (sum(error_dum**2,  na.rm = T)/sum(!is.na(error_dum)))
print(paste('total_rmse:',total_rmse))
print(paste('total_R2:',total_R2))
