...
data.frame(cbind(nbr_films_watched=test$nbr_films_watched,target_test,pred_y)) %>% 
  mutate(error = target_test - pred_y) %>% 
  mutate(films_watch_bin = cut(nbr_films_watched, breaks=c(1,20,50,150,500,1500,100000))) %>% 
...
rf <- ranger(rating ~ . - user_id, data = train_rf)
