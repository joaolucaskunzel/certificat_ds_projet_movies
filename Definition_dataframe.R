...
set.seed(4321)
SelMov<-user_movie_df %>% group_by(movieId) %>% summarise(Nrate=n()) %>% filter(Nrate>Nfpu) %>% select(movieId)
...
nbr_vues_film_500 <- nbr_vues_film %>% filter (total_views>=500)
...
actors_by_user <-  user_movie_df %>% 
  ...
  pivot_wider(values_from = c(avg_act,sd_act,count_act), names_from = Actors, values_fill=FALSE)
#Director by user
# Problème DirIMdb devient vide
# NDirTop<-2
